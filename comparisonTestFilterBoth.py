...
recorded_game_results_file = "game_results/results_trans_100_v_filter_expanded.txt"
...
if response.status_code == 200:
    ...
    model = create_model(simplified=simplified)
                ...
                if message == "server: An error has occurred on the server side. Your game may become unstable or unplayable.":
                    ...
                    if action_required:
                        if num_times_a_timeout_occurred > max_timeouts:
                            ...
                            if game.active_player == game.player_one.name:
                                choose_completely_randomly = False
                                filter_moves_arg = filter_moves
                            else:
                                choose_completely_randomly = True
                                filter_moves_arg = filter_moves
                            outputs = model_make_choice(model, game, training=training, choose_random=choose_completely_randomly, filter_moves=filter_moves_arg)
