import keras
import asyncio
import websockets
import requests
from GameObject import Game
import ast
import datetime
from model_definitions import create_model, model_make_choice, resolve_training, determine_reward_of_move_just_made
import os
import shutil
import DeckManipulation
import miniDeckbuilder
import traceback
from protocolSettings import bot_room_name, ws_protocol, site_name, training, URL, name_1, name_2


simplified = True
transformer = False
use_custom_move_rewards = False
cards_changed_each_time = 0
num_games = 500
game_count = 0
initial_game_number = 0
num_times_a_timeout_occurred = 0
max_timeouts = 20
file_name_model = "trained_models/RevisedSimpleModel250.keras"
valid_deck_names = ["CatoChamp"]
game_count_file = "num_games_done_tracker.txt"
file_times_of_each_game = "game_times.txt"
shutil.copytree("default_decks", "decks", dirs_exist_ok=True)
if not os.path.exists("decks/CatoCore"):
    raise SystemError
if not os.path.exists("trained_models"):
    os.mkdir("trained_models")
if not os.path.exists(file_times_of_each_game):
    open(file_times_of_each_game, "w").close()
if not os.path.exists(game_count_file):
    open(game_count_file, "w").close()
if training:
    if os.path.exists(game_count_file):
        with open(game_count_file, "r") as f:
            file_content = f.read()
            try:
                game_count = game_count + int(file_content)
                initial_game_number = game_count
            except ValueError:
                pass

data = {
    "name1": name_1,
    "name2": name_2,
    "id": bot_room_name,
    "private": False
}

response = requests.post(URL + "api/create_bot_room/", data=data)
print(response)
if response.status_code == 200:
    received_data = response.json()
    print(received_data)
    game_id = received_data["id"]
    game_url = ws_protocol + "://" + site_name + "/ws/play/" + game_id + "/"
    game = Game(name_1, name_2)
    model = create_model(simplified=simplified, transformer=transformer)
    if os.path.exists(file_name_model):
        model = keras.models.load_model(file_name_model, safe_mode=False)
    optimizer = keras.optimizers.Adam(learning_rate=0.001, clipnorm=1.0)
    game_finished = False
    performing_eor_model_training = False

    inputs_per_player = {1: [], 2: []}
    action_idx_per_player = {1: [], 2: []}
    rewards_per_player = {1: [], 2: []}
    rewards_stay_fix = {1: [], 2: []}
    waiting_time = 0  # milliseconds
    start_time_all_games = datetime.datetime.now()
    start_time_current_game = start_time_all_games
    seconds_timeout = 3

    async def receive_messages(websocket, send_queue, done_event):
        global game_finished
        global waiting_time
        global inputs_per_player
        global action_idx_per_player
        global rewards_per_player
        global rewards_stay_fix
        global game
        global game_count
        global num_times_a_timeout_occurred
        global start_time_current_game
        global performing_eor_model_training
        async for message in websocket:
            try:
                message = ast.literal_eval(message)["message"]
                print("Received message:", message)
                if "/slowdown " in message:
                    idx = message.index("/slowdown ")
                    end = idx + len("/slowdown ")
                    waiting_time = int(message[end:])
                if message == "server: An error has occurred on the server side. Your game may become unstable or unplayable.":
                    game_finished = True
                    message = "CHAT_MESSAGE//reset-game"
                    message = '{"message": \"' + message + '\"}'
                    await websocket.send(message)
                    num_times_a_timeout_occurred = 0
                    game_finished = False
                    game = Game(name_1, name_2)
                    inputs_per_player = {1: [], 2: []}
                    action_idx_per_player = {1: [], 2: []}
                    rewards_per_player = {1: [], 2: []}
                    rewards_stay_fix = {1: [], 2: []}
                else:
                    action_required = game.update_game_from_game_update_message(message)
                    if action_required:
                        if num_times_a_timeout_occurred > max_timeouts:
                            game_finished = True
                            message = "CHAT_MESSAGE//reset-game"
                            message = '{"message": \"' + message + '\"}'
                            await websocket.send(message)
                            num_times_a_timeout_occurred = 0
                            game_finished = False
                            game = Game(name_1, name_2)
                            inputs_per_player = {1: [], 2: []}
                            action_idx_per_player = {1: [], 2: []}
                            rewards_per_player = {1: [], 2: []}
                            rewards_stay_fix = {1: [], 2: []}
                        elif action_required == "EOR_TRAINING" and len(inputs_per_player[1]) > 2:
                            print("Begin EOR processing.")
                            performing_eor_model_training = True
                            start_time = datetime.datetime.now()
                            if training:
                                resolve_training(
                                    model, game, optimizer, inputs_per_player, action_idx_per_player, rewards_per_player, rewards_stay_fix, game_completed=False
                                )
                            print("Finished EOR processing.")
                            end_time = datetime.datetime.now()
                            if (end_time - start_time) < datetime.timedelta(seconds=3):
                                performing_eor_model_training = False
                            print(end_time - start_time)
                        elif action_required == "FINISHED" and len(inputs_per_player[1]) > 2:
                            game_finished = True
                            num_times_a_timeout_occurred = 0
                            message = "CHAT_MESSAGE//reset-game"
                            message = '{"message": \"' + message + '\"}'
                            await websocket.send(message)
                            print("Begin EOG processing.")
                            start_time = datetime.datetime.now()
                            if training:
                                resolve_training(model, game, optimizer, inputs_per_player, action_idx_per_player, rewards_per_player, rewards_stay_fix)
                            print("Finished EOG processing.")
                            end_time = datetime.datetime.now()
                            print(end_time - start_time)
                            game_count += 1
                            print("games completed:", game_count)
                            if training and game_count < 1000:
                                with open(file_times_of_each_game, "a") as f:
                                    f.write(str(game_count) + ", " + str((end_time - start_time_current_game).total_seconds()) + "\n")
                                    f.close()
                            start_time_current_game = end_time
                            if training:
                                with open(game_count_file, "w") as f:
                                    f.write(str(game_count))
                                    f.close()
                            if game_count % 25 == 0:
                                model.save(file_name_model)
                                model.save(file_name_model.replace(".keras", "") + str(game_count) + ".keras")
                            if game_count < num_games:
                                game_finished = False
                                game = Game(name_1, name_2)
                                inputs_per_player = {1: [], 2: []}
                                action_idx_per_player = {1: [], 2: []}
                                rewards_per_player = {1: [], 2: []}
                                rewards_stay_fix = {1: [], 2: []}
                            else:
                                await asyncio.sleep(0.2)
                                done_event.set()
                                await websocket.close()
                                return
                        elif action_required == "LOADDECK":
                            deck_name = DeckManipulation.choose_deck(valid_deck_names)
                            deck_content = miniDeckbuilder.load_adjust_deck(deck_name, cards_changed_each_time)
                            DeckManipulation.send_deck(game.active_player, deck_content)
                            message = "CHAT_MESSAGE//loaddeckbot/" + game.active_player + "/" + deck_name
                            # message = "CHAT_MESSAGE//loadrandombot/" + game.active_player
                            message = '{"message": \"' + message + '\"}'
                            await send_queue.put(message)
                        elif not game_finished:
                            outputs = model_make_choice(model, game, training=training)
                            action_idx = outputs["action_idx"]
                            current_player = outputs["player_choosing"]
                            choice = outputs["choice"]
                            if outputs["flag"] == "Multi":
                                inputs = outputs["state"]
                                value_estimate = outputs["critic_value"]

                                inputs_per_player[current_player].append(inputs)
                                action_idx_per_player[current_player].append(action_idx)
                                reward = 0
                                if use_custom_move_rewards:
                                    reward = determine_reward_of_move_just_made(game, choice)
                                rewards_per_player[current_player].append(reward)
                                rewards_stay_fix[current_player].append(reward != 0)

                            message = game.create_automated_choice_message(choice)
                            await send_queue.put(message)
            except Exception as e:
                print("error:", e)
                with open("errors.txt", "a") as f:
                    f.write(traceback.format_exc())


    async def send_messages(websocket, send_queue, done_event):
        global waiting_time
        global num_times_a_timeout_occurred
        global game_finished
        global performing_eor_model_training
        global seconds_timeout
        while not done_event.is_set():
            try:
                message = await asyncio.wait_for(send_queue.get(), timeout=seconds_timeout)
            except asyncio.TimeoutError:
                print("timed out")
                num_times_a_timeout_occurred += 1
                if not game_finished and not performing_eor_model_training:
                    message = "CHAT_MESSAGE//force-send-auto-data"
                    message = '{"message": \"' + message + '\"}'
                    print("send time out message:", message)
                    await websocket.send(message)
                performing_eor_model_training = False
                continue
            await asyncio.sleep(waiting_time / 1000)
            await websocket.send(message)
            send_queue.task_done()
            print("sent:", message)

    async def connect():
        send_queue = asyncio.Queue()
        done_event = asyncio.Event()
        async with websockets.connect(game_url) as websocket:
            await asyncio.gather(
                receive_messages(websocket, send_queue, done_event),
                send_messages(websocket, send_queue, done_event),
                return_exceptions=True
            )
    asyncio.run(connect())
    if training:
        model.save(file_name_model)
    end_time_all_games = datetime.datetime.now()
    print("Time taken to play all " + str(num_games - initial_game_number) + " games: ", end_time_all_games - start_time_all_games)
